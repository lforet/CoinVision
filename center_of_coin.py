...
cv.ShowImage("Image",final_image)
...
while contour:
	...
	print i, "  bound_rect,", bound_rect,"  list(contour)",  list(contour)
	#print "contour i = ", contour[i]
	i = i +1	
	# Get the size of the contour
	size = abs(cv.ContourArea(contour))
	if size > big_size:
		big_size = size 
		big_contour = bound_rect
	# Is convex
...
print "largest contour = ", big_contour
print "size = ", big_size
pt1 = (big_contour[0], big_contour[1])
pt2 = (big_contour[0] + big_contour[2], big_contour[1] + big_contour[3])
points.append(pt1)
points.append(pt2) 
cv.Rectangle(img, pt1, pt2, cv.CV_RGB(255,0,0), 3)
